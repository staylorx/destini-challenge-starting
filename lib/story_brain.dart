...
class StoryBrain {
  //Uncomment the lines below to include storyData as a
  //private property in StoryBrain. Hint: You might need to change
  //something in story.dart to make this work.

  //Step 16 - Create a property called storyNumber which starts with a
  //value of 0. This will be used to track which story the user is currently viewing.
  //Step 25 - Change the storyNumber property into a private property so that
  //only story_brain.dart has access to it. You can do this by right clicking
  //on the name (storyNumber) and selecting Refactor -> Rename to make the
  //change across all the places where it's used.
  int _storyNumber = 0;
...
//Step 23 - Use the storyNumber property inside getStory(), getChoice1() and getChoice2() so that it gets the updated story and choices rather than always just the first (0th) one.
  ...
  void nextStory(int storyChoiceNumber) {
    logger.d("Story $_storyNumber, choice $storyChoiceNumber");

    //get the story choice at the _storyNumber
    late int choice;

    if (storyChoiceNumber == 1) {
      choice = _storyData[_storyNumber].choice1Goto;
    } else if (storyChoiceNumber == 2) {
      choice = _storyData[_storyNumber].choice2Goto;
    } else {
      logger.e("story choice is $storyChoiceNumber, which shouldn't happen.");
    }

    if (choice < 0) {
      //entirely done
      logger.i("The story is complete.");
      restart();
    } else {
      _storyNumber = choice;
    }
...
//Step 27 - Create a method called buttonShouldBeVisible() which checks to see if storyNumber is 0 or 1 or 2 (when both buttons should show choices) and return true if that is the case, else it should return false.
  bool buttonShouldBeVisible(int buttonNumber) {
    late bool result;

    if (buttonNumber == 1) {
      result = _storyData[_storyNumber].choice1Goto >= 0;
    }
    if (buttonNumber == 2) {
      result = _storyData[_storyNumber].choice2Goto >= 0;
    }

    logger
        .d('story $_storyNumber button $buttonNumber visible returns $result');
    return result;
  }
}
